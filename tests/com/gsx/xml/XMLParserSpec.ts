...
import { NormalXMLParser } from 'com/gsx/xml/NormalXMLParser';
...
describe('XML Parser Test', (): void => {
    ...
    beforeAll((): void => {
        // this._parser = new NormalXMLParser();
    });

    it('Parse xml string to the document object', (done: DoneFn): void => {
        parser.parse(`
            <f:Form xmlns:f="com/gsx/components/form"
                xmlns:m="com/gsx/components/mobile">
                <f:TextBox></f:TextBox>
                <f:Button></f:Button>
                <m:List>
        ...
        `).then((xml: Document): void => {
            ...
            expect(form.namespaceURI).toBe('com/gsx/components/form');
            ...
            expect(uiswitch.namespaceURI).toBe('com/gsx/components/mobile');
            done();
        });
    });
